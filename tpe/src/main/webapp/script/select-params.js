$(document)
		.ready(
				function() {
					...
					// item is selected
					/*
					 * $("select#select_output").click(function() { // Get the
					 * selected output id or text out = $(this).val();
					 * console.log('Selected out: ' + out);
					 * $("#params_out").load(href_output, { // Pass the
					 * parameters to the action to return the params div
					 * selectedOutput : out }, function() {
					...
					$("select#select_output")
							.change(
									function() {
										...
										outputValue = $("select#select_output")
												.val();
					...
					/**
					 * This function loads all the sub variables depending on
					 * the selected TPE output selected
					 */
					function loadVariables(otext) {
						...
						case 'TPE':
							 ...
							 * loadSubregions(); // Automatically reload the
							 * google map
							 * $("select#select_regions").change(function() { //
							 * Load the Google Map, the selected sub regions
							 * changes initializeGoogleMap();
							 * 
						...
						case 'SOIL':
							/*
							 * // console.log('TPE SELECTED: '+ot); // Load the
							 * sub regions for the selected country
							 * loadSubregions(); // Automatically reload the
							 * google map
							 * $("select#select_regions").change(function() { //
							 * Load the Google Map, the selected sub regions
							 ...
							 * });
							 */
							// Automatically reload the Google Map when the soil
							// properties
							// change
							/*
					...
					/**
					 * This function loads the years based on the selected TPE
					 * output parameter
					 */
					function loadYears() {
						// Get the preselected country id
						...
						if (outputText == 'TPE') {
							// If TPE was selected, load the years based on the
							// selected crop
							...
							// If CLIMATE was selected, then load the years
							// based on the
							// selected country only
							// TODO To add more params
							// Set the cultivar to null, because CLIMATE was
							// selected.
							_cultivar = null;
