...
public class StatisticsUtils {
	...
	public static BigDecimal average(Number[] numbers) {
		...
		try {
			...
			result = sum.divide(new BigDecimal(numbersCount), 10,
					RoundingMode.HALF_EVEN);// Maximum of 10 decimal digits to
											// avoid periodic number exception.
		}
		return result;
	}
	 ...
	 */
	public static BigDecimal[] numbersArrayToSortedBigDecimalArray(
			Number[] numbers) {
		if (numbers == null) {
			return null;
		}
	...
	private static BigDecimal average(final BigDecimal sum,
			final BigDecimal numbersCount) {

		BigDecimal result;
		try {
			result = sum.divide(numbersCount);
		} catch (ArithmeticException ex) {
			result = sum.divide(numbersCount, 10, RoundingMode.HALF_EVEN);// Maximum
																			// of
																			// 10
																			// decimal
																			// digits
